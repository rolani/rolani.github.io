fetch("repos.json")
  ...
  .then((data) => {
    ...
    data.forEach(function (repo) {
        ...
        repo.date.split("T")[0]
      }</span><div class="mdl-layout-spacer"></div><div class="mdl-card__actions"><img class="mr-5"src="img/star.svg"/></div></div></div></div><!--repo card ended-->`;
        output += `<!--repo card started--><div class="col-sm-4 mb-40"><div class="mdl-card mdl-shadow--2dp pa-0 repo-card"><div class="mdl-card__title pa-0"><img class="blog-img"loading="lazy"src="${
          ...
          repo.date.split("T")[0]
        }</span><div class="mdl-layout-spacer"></div><div class="mdl-card__actions"><img class="mr-5"src="assets/static/images/star.svg"/></div></div></div></div><!--repo card ended-->`;
    });
    document.getElementById("repo-card").innerHTML = output;
  });
    fetch("assets/static/json/repos.json")
      .then((res) => res.json())
      .then((data) => {
        let output = "";
        data.forEach(function (repo) {
